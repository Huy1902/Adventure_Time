...
void FontManager::drawText(const char*text, int x, int y)
{
	SDL_Surface* surface = TTF_RenderText_Solid(mFont, text, mColor);
	SDL_Texture* texture = SDL_CreateTextureFromSurface(mRenderer, surface);

	SDL_Rect messageRect = { x, y, 0, 0 };
	TTF_SizeText(mFont, text, &messageRect.w, &messageRect.h);
